...
@section('content')
    <div class="d-flex flex-column">
        <h4 class="text-lg font-bold">{{ trans('admin.Products') }}</h4>
        <ul class="flex items-center text-sm text-gray-500">
            <li><a href="#"> {{ trans('admin.Dashborad') }} </a></li>-
            <li><a href="#"> {{ trans('admin.Products') }} </a></li>-
            <li><a href="#"> {{ trans('admin.Edit') }} </a></li>
        </ul>
    </div>
    <div class="max-w-md mx-auto mt-10 bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-2xl font-bold mb-4">{{ trans('admin.Edit') }} {{ trans('admin.Product') }}</h2>
        <form action="{{ route('admin.product.update', ['product' => $product]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                <label for="provider" class="block text-sm font-medium text-gray-700">{{ trans('admin.Provider') }}</label>
                <select name="provider" id="provider" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    ...
                    @foreach ($providers as $provider)
                        <option value="{{ $provider->id }}" {{ old('provider', $product->id_provider) == $provider->id ? 'selected' : '' }}>{{ $provider->name }}</option>
                    @endforeach
                </select>
                @error('provider')
                    <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full h-28 shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('description', $product->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-4">
                <label for="price" class="block text-sm font-medium text-gray-700">{{ trans('admin.Price') }}</label>
            ...
            <div class="mb-4">
                <label for="stock" class="block text-sm font-medium text-gray-700">{{ trans('admin.Stock') }}</label>
                <input type="checkbox" name="stock" id="stock" {{ old('stock', $product->stock) ? 'checked' : '' }} class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block shadow-sm sm:text-sm border-gray-300 rounded-md">
            </div>
